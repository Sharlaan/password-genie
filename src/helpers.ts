...
const getRandomCharFromSet = (set: string) =>
  set.charAt(floor(random() * set.length));
...
export function countRepeatedChar(str: string) {
  ...
  return (str.match(pattern) || []).reduce(
    (count, repetition) => count + (repetition.length - 1),
    0,
  );
}
  ...
  return str.split('').reduce((count, char, i) => {
    const condition =
      ...
      ((isLetter(char) &&
        isLetter(str[i - 1]) &&
        str[i - 1].toLowerCase() ===
          LETTERS[LETTERS.indexOf(char.toLowerCase()) - 1]) ||
        (isNumber(char) &&
          isNumber(str[i - 1]) &&
          Number(str[i - 1]) === Number(char) - 1));
