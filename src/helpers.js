...
const getRandomCharFromSet = (set) => set.charAt(floor(random() * set.length));
const getRandomIntBetween = (min, max) => floor(random() * (max - min + 1) + min);
const range = (n) => [...Array(n)];
const isLetter = (char) => /[a-zA-Z]/.test(char);
const isLowerCaseLetter = (char) => /[a-z]/.test(char);
const isUpperCaseLetter = (char) => /[A-Z]/.test(char);
const isNumber = (char) => /\d/.test(char);
const isSymbol = (char) => SYMBOLS_EXTENDED.includes(char);
/**
...
export function pickDigits(n) {
    return range(n).map(() => getRandomIntBetween(0, 9));
}
export function pickSymbols(n, useAmbiguousSymbols) {
    return range(n).map(() => getRandomCharFromSet(!useAmbiguousSymbols ? SYMBOLS : SYMBOLS.concat(AMBIGUOUS)));
}
function pickAmbiguous(n) {
    return range(n).map(() => getRandomCharFromSet(AMBIGUOUS));
}
export function pickLetters(n) {
    return range(n).map(() => {
...
export function countConsecutiveSameType(str) {
    return str.split('').reduce((count, char, i) => {
        const condition = ((isLowerCaseLetter(char) && isLowerCaseLetter(str[i - 1])) ||
...
export function countRepeatedChar(str) {
...
export function countSequentialForward(str) {
    return str.split('').reduce((count, char, i) => {
        const condition = i !== 0 && ((isLetter(char) && isLetter(str[i - 1]) &&
...
export function countSequentialReverse(str) {
    return str.split('').reduce((count, char, i) => {
        const condition = i !== 0 && ((isLetter(char) && isLetter(str[i - 1]) &&
            str[i - 1].toLowerCase() === LETTERS[LETTERS.indexOf(char.toLowerCase()) + 1]) ||
            (isNumber(char) && isNumber(str[i - 1]) && Number(str[i - 1]) === Number(char) + 1));
